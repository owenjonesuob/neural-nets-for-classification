...
\begin{abstract}
    ...
    Section \ref{sec:demos} of this paper
    
    We conclude by highlighting some areas for possible further work
...
\section{Demonstration of Methods} \label{sec:demos}
    ...
    \subsection{Binary Classification}
        ...
        Adding a very small hidden layer to the model is enough to capture the non-linear decision boundary almost perfectly, as can be seen in the right-hand plot of Figure \ref{fig:moons}. This model achieves over 99\% accuracy after 200 epochs of training.
        
        
        \begin{figure}[h]
            \begin{tabular}{cc}
                \subfloat{\includegraphics[width=0.45\textwidth]{moons_logreg}} &
                \subfloat{\includegraphics[width=0.45\textwidth]{moons_nnet}}
            \end{tabular}
            \caption{The \textit{left-hand} plot shows the decision boundary found by logistic regression after 200 training epochs. The \textit{right-hand} plot shows the decision boundary found by a small neural network after 200 training epochs. All hyperparameters are the same in both models.}
            \label{fig:moons}
        \end{figure}
        
        
        We should take care, however, not to define a model which is unnecessarily complex for the task at hand; and not to overtrain our model once an adequate level of performance has been achieved, since otherwise we are likely to encounter overfitting as described in \ref{sec:improvements}. Figure \ref{fig:tricky} shows the consequences of overtraining a model and how early stopping can help to prevent this from happening.
        
        
        \begin{figure}[h]
            \begin{tabular}{ccc}
                \subfloat{\includegraphics[width=0.3\textwidth]{tricky_over_train}} &
                \subfloat{\includegraphics[width=0.3\textwidth]{tricky_over_val}} &
                \subfloat{\includegraphics[width=0.3\textwidth]{tricky_over_curves}} \\
                \subfloat{\includegraphics[width=0.3\textwidth]{tricky_early_train}} &
                \subfloat{\includegraphics[width=0.3\textwidth]{tricky_early_val}} &
                \subfloat{\includegraphics[width=0.3\textwidth]{tricky_early_curves}}
            \end{tabular}
    ...
    \subsection{Multiclass Classification}
        ...
        \begin{figure}[h]
            \begin{tabular}{ccc}
                \subfloat{\includegraphics[width=0.3\textwidth]{islands_early_val}} &
                \subfloat{\includegraphics[width=0.3\textwidth]{islands_late_val}} &
                \subfloat{\includegraphics[width=0.3\textwidth]{islands_curves}}
            \end{tabular}
            \caption{Early stopping implemented as described in Section \ref{sec:early_stopping}. When $s = 80$, training terminated after 434 epochs and weights were reset to the values they held after epoch 402 (left). When $s = 800$, training terminated after 2110 epochs and weights were reset to the values they held after epoch 1409 (centre). These saved epochs correspond to local minima of the validation cost function, in orange (right).}
            \label{fig:islands}
        \end{figure}
        
        
        For the remainder of this section we will consider the Kuzushiji-MNIST (KMNIST) dataset \footnote{Dataset available at \url{https://www.kaggle.com/anokas/kuzushiji}.} of handwritten Japanese \textit{hiragana} characters. More information and background on the dataset can be found in \cite{kmnist}.
        
        KMNIST is designed as a drop-in replacement for the infamous MNIST dataset of handwritten numeric digits, and therefore is identical in structure to MNIST: each observation is a single 28x28 matrix representing a 28x28-pixel greyscale image. Each element of the matrix represents a pixel, and its value represents the "intensity" of that pixel on a scale of 0 (white) to 1 (black). Some examples from the dataset are shown in Figure \ref{fig:kmnist_examples}.
